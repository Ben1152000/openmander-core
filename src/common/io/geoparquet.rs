...
pub(crate) fn write_to_geoparquet(path: &Path, geoms: &Vec<geo::MultiPolygon<f64>>) -> Result<()> {
...
pub(crate) fn read_from_geoparquet(path: &Path) -> Result<Vec<geo::MultiPolygon<f64>>> {
    ...
    let gp_meta = builder.geoparquet_metadata()
