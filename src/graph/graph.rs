...
impl Graph {
    ...
    #[inline] pub(crate) fn len(&self) -> usize { self.size }

    /// Check if the graph is empty (no nodes).
    #[inline] pub(crate) fn is_empty(&self) -> bool { self.size == 0 }

    /// Get the number of edges in the graph.
    #[inline] pub(crate) fn num_edges(&self) -> usize { self.edges.len() }
    ...
    #[inline] pub(crate) fn is_isolated(&self, node: usize) -> bool { self.range(node).is_empty() }
