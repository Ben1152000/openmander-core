...
impl FrontierSet {
    ...
    pub(super) fn new(num_parts: usize, num_nodes: usize) -> Self {
        Self {
    ...
    pub(super) fn rebuild(&mut self, assignments: &[u32], boundary: &[bool]) {
    ...
    pub(super) fn refresh(&mut self, u: usize, part: u32, on_boundary: bool) {
    ...
    #[inline] pub(super) fn contains(&self, u: usize) -> bool { self.index[u].is_some() }

    /// Read-only view of frontier nodes for part p.
    #[inline] pub(super) fn get(&self, part: u32) -> &[usize] { &self.lists[part as usize] }
